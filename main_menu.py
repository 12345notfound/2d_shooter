...
def main_window():
    ...
    for i in range(1, 7):
        if i == 1:
            button.append(Button_rect(screen, location_objects['1'][0],
                                      location_objects['1'][1], 200, 187,
                                      lambda:
                                      print(i)))
            screen.blit(lvl_text[0], (0, 0, 1400, 700))
            for j in range(lvl[i - 1]):
                screen.blit(star, (
                    location_objects['звезда'][0] + j * 53 + (i - 1) // 2 * 233,
                    location_objects['звезда'][1] + 213 * ((i - 1) % 2), 1400,
                    700))
            ...
            if lvl[i - 2] != 0:
                button.append(Button_rect(screen, location_objects['1'][0] + (
                            i - 1) // 2 * 233,
                                          location_objects['1'][1] + 213 * (
                                                      (i - 1) % 2), 200, 187,
                                          lambda: open_now(1)))
                screen.blit(lvl_text[i - 1], (0, 0, 100, 100))
                for j in range(lvl[i - 1]):
                    screen.blit(star, (
                        location_objects['звезда'][0] + j * 53 + (
                                    i - 1) // 2 * 233,
                        location_objects['звезда'][1] + 213 * ((i - 1) % 2),
                        1400, 700))
            else:
                screen.blit(lock, (
                    location_objects['замок'][0] + (i - 1) // 2 * 233,
            ...
            screen.blit(lock, (
                location_objects['замок'][0] + (i - 1) // 2 * 233,
                location_objects['замок'][1] + 213 * ((i - 1) % 2),
                130,
                130))
